(() => {
  /**
   * Chip hues for the light paper theme. Every entry clears 4.5:1 contrast
   * against the chip background (#efe7da) and the pane background (#fffaf2).
   */
  const PALETTE = ["#155e75", "#4338ca", "#92400e", "#be123c", "#065f46", "#7e22ce", "#1d4ed8"];
  ...
  function renderChips(tokens, root, opts = {}) {
    ...
    for (const token of tokens) {
      ...
      el.className = "chip";
      const isUnk = unk.has(token) || token === "[UNK]";
      if (isUnk) el.classList.add("unk");
      if (token === " " || token === "\n") el.classList.add("space");
      el.textContent = visible(token);
      el.title = `U+${[...token].map((c) => (c.codePointAt(0) ?? 0).toString(16).toUpperCase().padStart(4, "0")).join(" ")}`;
      // UNK keeps its own alarm colour from CSS; inline hues would override it.
      if (!isUnk) {
