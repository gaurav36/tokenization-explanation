(() => {
  ...
  const inspectPresetButtons = [...document.querySelectorAll("[data-inspect-preset]")];
  ...
  function updateInspect() {
    ...
    inspectPresetButtons.forEach((btn) => {
      const isActive = btn.getAttribute("data-inspect-preset") === text;
      btn.setAttribute(
        "aria-pressed",
        isActive ? "true" : "false",
      );
  ...
  function updateLength() {
    ...
    const rows = [
      ["Word", nWord, "#155e75"],
      ["Naive 3-char (not BPE)", nSub, "#4338ca"],
      ["Character", nChar, "#92400e"],
      ["Byte", nByte, "#be123c"],
    ];
  ...
  function setCurrent() {
    if (performance.now() < suppressSetCurrentUntil) return;
    const y = getNavOffset();
    const navH = navBar?.getBoundingClientRect().height ?? 0;
    const tolerance = Math.max(56, Math.round(navH * 0.7));
    let current = sections[0];
    for (const sec of sections) {
      const top = sec.getBoundingClientRect().top;
      if (top - y <= tolerance) current = sec;
    }
  ...
  navLinks.forEach((a) => {
    a.addEventListener("click", () => {
      ...
      const distance = Math.abs(target?.getBoundingClientRect().top ?? 0);
      const suppressMs = Math.min(2200, Math.max(900, Math.round(distance * 1.2)));
      suppressSetCurrentUntil = performance.now() + suppressMs;
      window.clearTimeout(currentSyncTimer);
      currentSyncTimer = window.setTimeout(() => {
        suppressSetCurrentUntil = 0;
        setCurrent();
      }, suppressMs + 40);
    });
  });
